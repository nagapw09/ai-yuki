# Роадмап

Порядок фаз выведен из ТЗ §38–§40. Каждая фаза заканчивается работающим приложением,
а не «слоем без UI».

## Фаза 0 — Каркас ✅

- [x] Монорепо по структуре ТЗ §43 (npm workspaces + cargo workspace)
- [x] Tauri 2 shell, собирается на Windows и macOS
- [x] Design System: токены, типографика, 4/8px сетка, motion (§14)
- [x] Orbital-экран: Orb со всеми состояниями, Command Bar, rail (§13)
- [x] `SystemAdapter` / `FileAdapter` / `InputAdapter` / `ScreenAdapter` — трейты + обе реализации (§30)
- [x] SQLite-схема на все таблицы §31, секреты в OS secure storage (§29)
- [x] Типы ядра: Tool, ToolResult, Permission, RiskLevel, Task, Capability (§5, §21, §22, §32)
- [x] i18n ru/en/uk (§36)

## Фаза 1 — MVP-ядро (ТЗ §38)

- [x] AI Provider Layer: Claude, OpenAI, Gemini + всё, что говорит на протоколе
      OpenAI — OpenRouter, xAI, Ollama, LM Studio, Custom API. Стриминг SSE,
      вызовы инструментов, `Test Connection` (§4)
- [x] Agent Loop целиком: PLAN → EXECUTE → OBSERVE → VERIFY (§5)
- [x] Chat как вторичный экран: markdown, блоки кода, статусы инструментов (§15)
- [x] Tools: приложения, окна, файлы, буфер обмена, клавиатура, мышь, экран (§6, §8)
- [x] Permission Gate + Confirmation modal с показом плана (§21, §22)
- [x] Activity Log (§23)
- [x] Memory: short-term / session / long-term / episodic + UI просмотра,
      правки, удаления и полной очистки; долгосрочные факты уходят в контекст
      запроса, краткосрочные протухают по сроку (§9)
- [x] Hotkeys, Notifications, Reminders: глобальный хоткей вызова, планировщик
      напоминаний с повторами, системные уведомления (§25)

Проверено на Windows 11 сквозным прогоном через локальный мок-провайдер:
стрим, сборка вызова инструмента из фрагментов SSE, выполнение в Rust, возврат
результата модели, подтверждение опасного действия — и отказ, и разрешение,
сохранение факта в память, срабатывание напоминания по сроку, глобальный хоткей
из свёрнутого состояния.

Не проверено визуально: сам toast на Windows. Он требует ярлыка с
AppUserModelID, которого нет у запуска из `target/debug` — проверять нужно
на установленной сборке.

## Фаза 2 — Голос и зрение (ТЗ §38)

- [x] Pipeline: Microphone → VAD → STT → Agent → TTS → Speaker (§10)
- [x] Push-to-talk, перебивание, синтез речи средствами ОС
- [x] Wake word «Yuki» / «Hey Yuki» — **бюджет 300 мс из §37 выдержан**, если
      записать обращение в настройках: признаки MFCC и динамическое выравнивание
      сравнивают поток с образцами локально, отзыв приходит в момент
      произнесения слова, а фразы без обращения вообще не уходят на
      распознавание. Цена способа названа прямо: он зависит от голоса
      (`rust/voice/src/wake.rs`). Без записи работает прежний путь — обращение
      ищется в распознанном тексте, и это медленнее бюджета
- [ ] Streaming STT: сейчас фраза уходит на распознавание целиком
- [x] Screenshots + понимание экрана vision-моделью: снимок уходит модели
      картинкой во всех трёх диалектах, в журнал — только размеры (§6)
- [x] Accessibility tree как приоритетный источник, CV — fallback: на Windows
      через UI Automation; macOS ждёт живой проверки статуса TCC (§6)

Проверено на живом микрофоне утилитой `cargo run -p yuki-voice --example mic_check`
и на живом окне — `cargo run -p yuki-accessibility --example ui_check`.
Она же нашла и то, чего не видно из кода: фон тихой комнаты (RMS до 0.05) выше
любого разумного фиксированного порога, поэтому VAD считал шум речью непрерывно
и фраза не заканчивалась никогда. Пороги стали относительными — от нижнего
перцентиля громкости за две секунды, с прогревом и потолком, чтобы в шумной
комнате Yuki не оглохла.

Зрение проверено сквозным прогоном: снимок экрана на 408 КБ дошёл до провайдера
одной картинкой в правильном формате. Дерево интерфейса того же окна занимает
1155 символов вместо 408 КБ и содержит готовые действия — поэтому оно и стоит
в описаниях инструментов первым, а снимок объявлен запасным путём.

## Фаза 3 — Расширяемость (ТЗ §17–§20, §38)

- [x] MCP Manager: оба транспорта (stdio и HTTP), авторизация, Test Connection,
      рукопожатие, список и вызов инструментов (§19)
- [x] Capability Manager: манифест, health status, включение и удаление,
      проверка настоящим подключением (§18)
- [x] Capability Hub UI: Installed / Available Integrations / Add MCP (§17)
- [x] Инструменты самораcширения: Yuki сама ищет интеграцию, ставит её,
      проверяет и докладывает результат (§17, §42)
- [~] Сценарий «Юки, добавь возможность управлять Spotify» — механика готова
      и проверена на живом MCP-сервере, но запись Spotify в каталоге ведёт на
      сервер сообщества, который я не проверял. Установка это обнаружит сама:
      не поднялся — `health = failed` с причиной, а не «установлено» (§41.8)
- [x] Basic automation (§16): команды с триггерами (фраза, сочетание, запуск
      приложения, вручную), шагами-действиями, паузами и ветвлением if/else;
      переменные между шагами; редактор и запуск из интерфейса; Yuki создаёт
      команды сама через create_command
- [~] Browser Agent (§7): нативно — открытие ссылки и чтение страницы через
      дерево интерфейса; действия внутри страницы (клики, формы, извлечение) —
      через возможность «Браузер (Playwright)» из каталога. Встроенного
      Playwright нет: он тянет Node-рантайм и браузер в бандл, и это отдельное
      решение о поставке, а не строчка кода
- [x] Permission review до установки (§18): Hub показывает, к каким категориям
      получит доступ возможность, до согласия
- [ ] Sandbox для плагинов и разбор манифеста в изоляции (§18, §20): проверка
      по-прежнему сводится к факту успешного подключения

Проверено сквозным прогоном: `cargo run -p yuki-mcp --example mcp_check` на
настоящем stdio-сервере, затем добавление того же сервера через Hub (подключился,
инструменты видны), затем вызов его инструмента агентом — через Permission Gate
и подтверждение, 12 мс.

Автоматизации проверены отдельно: Yuki создала команду из трёх шагов по просьбе
в чате (с подтверждением, где виден весь план), команда сохранилась в базу и
выполнилась — результат первого шага попал в переменную, условие сработало,
подстановка `{{sys}}` дошла до буфера обмена.

## Фаза 4 — MVP-2 (ТЗ §39)

- [x] Ollama + LM Studio, Local Only режим (§29)
- [x] Plugin SDK и self-extension workflow (§20, §18)
- [x] Calendar: Google, Outlook (§25)
- [~] VRM avatar: восемь состояний, blink, мимика, idle-анимации, прозрачное
      окно поверх всех, click-through, resize и запоминание места. Lip-sync —
      приближение по фазе речи, а не по звуку: системный синтез (SAPI,
      AVSpeechSynthesizer) не отдаёт звуковой буфер, анализировать нечего
- [~] Advanced memory: семантический поиск на эмбеддингах поверх текстового.
      Консолидация эпизодов в долгосрочные факты не сделана
- [~] Advanced computer vision: область снимка и уменьшение до 1280 px.
      OCR не добавлен — тянуть tesseract в оба дистрибутива дорого, а
      accessibility-дерево закрывает большинство случаев точнее
- [~] Visual Command Builder: перестановка и дублирование шагов, ветвления,
      прогон с показом каждого шага. Полотна с перетаскиванием мышью нет

## Пропуски ТЗ, найденные сверкой с Astra (docs/GAPS.md)

В ТЗ их нет вовсе, но без красных пунктов приложение нельзя отдать человеку.

- [x] Мастер первого запуска с выдачей разрешений (§1) — включая живой опрос
      статуса TCC на macOS, которого не было с фазы 0
- [~] Auto-update (§2): плагин, кнопка в настройках и сборка выпусков для
      Windows и обеих архитектур macOS готовы. Канал выключен, пока не задан
      ключ подписи: его приватную половину нельзя ни положить в репозиторий,
      ни создать за владельца (docs/RELEASE.md)
- [x] Трей, фоновый режим, автозапуск, окно поверх остальных (§3)
- [x] Минимальные системные требования и живая сверка с машиной (§4)
- [x] Заметки (§5), погода и курсы (§6), роль и тон (§7), светлая тема (§8)
- [x] Библиотека готовых команд (§9), пресеты MCP (§10)
- [x] Экспорт и импорт (§11), диагностика (§14)
- [x] Требования к Remote Control (§12) — docs/REMOTE-CONTROL.md
- [x] Лицензирование (§13): решено — Yuki не продаётся, слоя лицензирования
      не будет (docs/LICENSING.md)
- [ ] UI-панели от плагинов (§8): нужна изоляция чужого кода в окне

## Фаза 5 — Version 2 (ТЗ §40)

- [ ] Remote control: Mobile/Web → Secure Gateway → Desktop, pairing по QR (§28)
- [ ] Telegram, cloud sync, proactive assistant (§34)
- [ ] Coding-agent integrations (§27)

## Критерии готовности MVP (ТЗ §41)

Сценарии, которые должны стабильно проходить на Windows **и** macOS:

1. «Юки, открой браузер и найди музыку для концентрации»
2. «Найди последний PDF в Downloads и открой его»
3. «Напомни мне завтра в 10:00 позвонить Ивану»
4. «Переведи выделенный текст»
5. «Запусти мой рабочий режим»
6. «Найди файл и перемести его в папку X»
7. «Открой приложение и выполни действие внутри него»
8. «Юки, добавь возможность управлять Spotify»

Бюджеты §37: startup < 3 с · wake word < 300 мс · first token < 1.5–2 с · UI 60 FPS.
